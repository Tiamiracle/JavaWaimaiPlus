...
export default {
	...
	methods: {
		...
		getDateDate() {
			//TODO 后端根据实际情况返回送达时间，接口原因后续优化
			//orderTime：主要用于用户选择使用，从当前时刻开始算起
			this.orderTime = nowTimeFormat();
			//服务端默认的送达时间：因为没有接口原因，暂时前端写为当前时间+1小时
			//presentFormat()函数：返回下一小时的时间
			this.arrivalTime = dayjs(presentFormat()).format('HH:mm')
		...
		// 星期几选择，如果是今日派送则时间由函数生成，否则使用列表写死的数据
		dateChange(index) {
			if (index === 1) {
				//明天派送，不包含第一个数据‘立即派送’
		...
		setTime(val) {
			if (val === '立即派送') {
			    ...
			    this.arrivalTime = `${String(newHour).padStart(2, '0')}:${String(minute).padStart(2, '0')}`;
			}
			this.setArrivalTime(this.arrivalTime)
		},
		touchstart(e) {
